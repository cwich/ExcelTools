...
my %field_column_names = (
	...
	'CD Exclusion Criterion' =>
	'Criterion for exclusion Continuous Delivery (CD)',
	...
	for my $worksheet ( $workbook->worksheets() ) {
		...
		foreach my $stage ( keys %stages ) {
			foreach my $automation_type ( keys %automation_types ) {
				$field_count{$stage}{$automation_type} = 0;
			}
		}
		my $artefact_count = 0;
		...
		for my $row ( $row_min .. $row_max ) {
			my $cell =
			  $worksheet->get_cell( $row, $field_column_position{'Project'} );
			next unless $cell;
			if ( $cell->value() eq "Yellow Pages Assets" ) {
				...
				foreach my $field ( keys %field_column_position ) {
					...
					my $cell =
					  $worksheet->get_cell( $row,
						$field_column_position{$field} );
					my $value = "";
					if ( defined($cell) ) {
						$value = $cell->value();
					}
					if ( exists $stages{$field}
						|| $field eq 'CD Exclusion Criterion' )
					{
						if ( $value eq "" || $value =~ /^ *$/ ) {
							$value = "None";
						}
		...
		foreach my $stage ( sort keys %field_count ) {
			print "  $stage\n";
			my $sum = 0;
			foreach my $autotype ( sort keys $field_count{$stage} ) {
				if ( $autotype ne "None" ) {
					my $value = $field_count{$stage}{$autotype};
					$sum += $value;
					printf( "    %-30s : %3i\n", $autotype, $value );
				}
			}
